...
async function run() {
  try {
    const user = await prisma.user.findUnique({ where: { email: "manager@justhob.com" }, include: { workspaceMemberships: true } });
    ...
    const lease = await prisma.lease.create({ data: { propertyId: prop.id, unitId: unit.id, tenantId: tenant.id, startDate: new Date(), endDate: new Date(), yearlyRent: 12000, status: "ACTIVE" }});
