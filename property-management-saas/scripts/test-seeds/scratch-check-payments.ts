...
async function main() {
  ...
  if (!pendingPayment) {
    console.log(
      "No pending payment found. Changing the first payment back to PENDING...",
    );
    if (payments.length > 0) {
      await prisma.payment.update({
        ...
        data: {
          status: "PENDING",
          amountPaid: 0,
          partialPaymentPromiseDate: null,
        },
      });
      console.log(
        "Successfully reset payment to PENDING so the user can test again.",
      );
    } else {
