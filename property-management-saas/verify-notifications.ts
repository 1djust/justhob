...
async function verify() {
  ...
  if (manager) {
    ...
    for (const n of mNotifs) {
      const icon = n.isRead ? "  " : "🔴";
      const typeIcon = n.type.includes("EXPIR")
        ? "📅"
      ...
      console.log(
        `      ${n.isRead ? "(Read)" : "(Unread)"} — ${n.createdAt.toLocaleString()}\n`,
      );
    }
  }
  ...
  const freeTenant = await prisma.user.findUnique({
    where: { email: "free-tenant@justhob.com" },
  });
  if (freeTenant) {
  ...
  for (const p of overduePayments) {
    ...
    console.log(
      `   Amount: ₦${p.amount.toLocaleString()} | Status: ${p.status}`,
    );
    console.log(
      `   Due: ${p.dueDate.toISOString().split("T")[0]} | Grace Period Ends: ${p.gracePeriodEnd?.toISOString().split("T")[0] || "N/A"}\n`,
    );
  }
  ...
  for (const r of reminders) {
    ...
    console.log(
      `   Tenant: ${r.payment.lease.tenant.name} | Sent: ${r.sentAt.toLocaleString()}\n`,
    );
  }
  ...
  console.log(
    "   Manager Dashboard: http://localhost:3000/login (manager@justhob.com / Test1234!)",
  );
