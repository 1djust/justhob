...
describe('Delinquency Escalation Protocol', () => {
  ...
  afterAll(async () => {
    // Cleanup
    await prisma.rentReminder.deleteMany({ where: { payment: { leaseId: testLeaseId } } });
    await prisma.notification.deleteMany({ where: { userId: testTenantUserId } });
  ...
  it('Test Case 1: 1-Day Overdue Warning', async () => {
    ...
    const payment = await prisma.payment.findUnique({ where: { id: paymentId }, include: { lease: { include: { tenant: true } } } });
  ...
  it('Test Case 4: 30-Day Complete App Lockout (Critical)', async () => {
    ...
    let reminderType = null;
    if (daysOverdue >= 30 && !payment!.evictionNoticeSent) reminderType = 'ACCOUNT_LOCKED';
