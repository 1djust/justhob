...
async function runQA() {
  ...
  try {
    ...
    console.log(
      "✅ Mailer logic triggered successfully (Check console logs above for mock output).\n",
    );
  } catch (err: unknown) {
  ...
  try {
    ...
    console.log(
      `✅ PDF generation logic completed. file saved to: ${testOutputPath}\n`,
    );
  } catch (err: unknown) {
  ...
  const simulateLimit = (plan: string, count: number, limit: number) => {
    if (count >= limit) {
      console.log(
        `✅ Limit correctly identified for ${plan}: ${count}/${limit} reached.`,
      );
    } else {
