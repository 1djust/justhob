...
async function setupTiers() {
  const users = [
    ...
    { email: 'enterprise-manager@justhob.com', plan: 'ENTERPRISE', name: 'Enterprise Manager' }
  ];
  ...
  for (const userData of users) {
    ...
    const userId = authData.user?.id || (await prisma.user.findUnique({ where: { email: userData.email } }))?.id;
    ...
    console.log(`✅ Success: ${userData.email} is ready on ${userData.plan} plan.`);
  }

  console.log('\n✨ All test users created!');
}

setupTiers().catch(console.error).finally(() => process.exit());
