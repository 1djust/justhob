...
async function setupTestTenants() {
  ...
  for (const t of tenants) {
    ...
    if (!manager || manager.workspaces.length === 0) {
      console.error(`❌ Manager ${t.managerEmail} or their workspace not found. Skipping.`);
      continue;
    }
    ...
    const userId = authData.user?.id || (await prisma.user.findUnique({ where: { email: t.email } }))?.id;
    ...
    let tenantProfile = await prisma.tenant.findFirst({ where: { email: t.email, workspaceId: workspace.id } });
    ...
    const lease = await prisma.lease.findFirst({ where: { tenantId: tenantProfile.id } });
    if (!lease) {
    ...
    console.log(`✅ Success: ${t.email} is ready to log into the mobile app and test!`);
  }

  console.log('\n✨ All test tenants created! Use password: Test1234!');
}

setupTestTenants().catch(console.error).finally(() => process.exit());
