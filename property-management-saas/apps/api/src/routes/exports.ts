  ...
  return new Promise<void>((resolve) => {
    ...
    doc.on("end", () => {
      ...
      reply
        ...
        .header(
          "Content-Disposition",
          `attachment; filename=${title.toLowerCase().replace(/\s+/g, "-")}-${Date.now()}.pdf`,
        )
        .send(result);
      resolve();
    });
    ...
    doc
      .fontSize(20)
      .text("PropertyStack - Secure Asset Center", { align: "center" });
    ...
    columns.forEach((col, i) => {
      doc.text(col.label, startX + i * colWidth, currentY, {
        width: colWidth,
        align: "left",
      });
    });
    ...
    rows.forEach((row) => {
      ...
      columns.forEach((col, i) => {
        ...
        doc.text(String(val || ""), startX + i * colWidth, currentY, {
          width: colWidth,
          align: "left",
        });
      });
  ...
  const requireEnterprise = async (request: any, reply: any) => {
    ...
    if (!workspace || workspace.plan !== "ENTERPRISE") {
      return reply.status(402).send({
        error:
          "Data Export is an Enterprise-only feature. Please upgrade your plan.",
      });
    }
  };
  ...
  server.get<{
    ...
    async (request, reply) => {
      ...
      const rows = tenants.map((t) => ({
        ...
        leaseStart: t.leases[0]?.startDate
          ? new Date(t.leases[0].startDate).toLocaleDateString()
          : "",
        leaseEnd: t.leases[0]?.endDate
          ? new Date(t.leases[0].endDate).toLocaleDateString()
          : "",
      ...
      reply.header(
        "Content-Disposition",
        `attachment; filename=tenants-${Date.now()}.csv`,
      );
  ...
  server.get<{
    ...
    async (request, reply) => {
      ...
      const rows = payments.map((p) => ({
        ...
        status: p.status,
        paymentMethod:
          (p as unknown as { paymentMethod?: string }).paymentMethod || "",
