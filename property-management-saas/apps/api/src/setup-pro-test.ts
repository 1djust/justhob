...
async function setupProTest() {
  ...
  const user = await prisma.user.findUnique({
    where: { email },
    include: { workspaces: { where: { role: 'PROPERTY_MANAGER' }, include: { workspace: true } } }
  });
  ...
  const currentProps = await prisma.property.count({ where: { workspaceId, deletedAt: null } });
  ...
  const properties = await prisma.property.findMany({ where: { workspaceId, deletedAt: null } });
  ...
  // 3. Setup 50 Tenants
  const currentTenants = await prisma.tenant.count({ where: { workspaceId, deletedAt: null } });
...
setupProTest().catch(console.error).finally(() => process.exit());
