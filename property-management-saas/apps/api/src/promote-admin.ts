...
async function main() {
  ...
  try {
    ...
    if (!user) {
      console.log(
        `User "${emailOrId}" not found in Prisma database. Creating new SUPER_ADMIN user...`,
      );

      const password = process.argv[3] || "Test1234!";
      console.log(`Creating user in Supabase Auth with password: ${password}`);

      const { data: supaUser, error: createSupaError } =
      ...
      if (createSupaError || !supaUser?.user) {
        ...
        if (
          createSupaError?.message?.includes("already exists") ||
          (createSupaError as any).status === 422
        ) {
          console.log(
            "User already exists in Supabase Auth. Attempting to retrieve credentials...",
          );
          ...
          if (existingSupaUser) {
            console.log(
              `Retrieved Supabase Auth user ID: ${existingSupaUser.id}. Creating database profile...`,
            );
            ...
            console.log(
              "Database profile successfully created and upgraded to SUPER_ADMIN.",
            );
        ...
        console.error(
          `Error creating user in Supabase Auth: ${createSupaError?.message || "Unknown error"}`,
        );
        process.exit(1);
      }

      console.log(
        `Successfully created user in Supabase Auth (ID: ${supaUser.user.id}).`,
      );
      ...
      console.log(
        `Success! New SUPER_ADMIN user "${newUser.email}" has been created and activated.`,
      );
      return;
    }

    console.log(
      `Found user: ${user.name || "N/A"} (${user.email}) - Current Role: ${user.role}`,
    );

    if (user.role === "SUPER_ADMIN") {
      console.log(
        "User is already a SUPER_ADMIN. Syncing Supabase metadata just in case...",
      );
    } else {
      // Update Prisma role
    ...
    const { data: supaData, error: fetchError } =
      await supabaseAdmin.auth.admin.getUserById(user.id);

    if (fetchError || !supaData?.user) {
      console.warn(
        `Warning: Could not fetch user from Supabase Auth: ${fetchError?.message || "User not found"}`,
      );
    } else {
      ...
      if (updateError) {
        console.error(
          `Error updating Supabase user metadata: ${updateError.message}`,
        );
      } else {
        console.log(
          "Supabase user metadata successfully updated with role: SUPER_ADMIN.",
        );
      }
    }
