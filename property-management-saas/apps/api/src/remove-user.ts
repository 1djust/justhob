...
async function main() {
  ...
  try {
    ...
    if (user) {
      console.log(`Found user in Prisma: ${user.name || "N/A"} (${user.email}) - Role: ${user.role}`);
    } else {
      console.log(`User "${emailOrId}" not found in Prisma. Checking Supabase Auth...`);
    }
    ...
    if (userId) {
      ...
      const { error: supaErr } = await supabaseAdmin.auth.admin.deleteUser(userId);
      if (supaErr) {
        console.warn(`Warning: Supabase Auth deletion returned error: ${supaErr.message}`);
      } else {
      ...
      if (listError) {
        ...
        if (foundSupaUser) {
          ...
          console.log(`Found orphaned user in Supabase: (${userEmail}) [ID: ${userId}]. Deleting...`);
          const { error: supaErr } = await supabaseAdmin.auth.admin.deleteUser(userId);
          if (supaErr) {
    ...
    if (!user && !supaUserDeleted) {
      console.error(`Error: User "${emailOrId}" could not be found in either Prisma or Supabase Auth.`);
      process.exit(1);
    }

    console.log(`Success! User "${userEmail || emailOrId}" has been permanently removed from the system.`);
  } catch (error: any) {
    console.error("An error occurred during user deletion:", error.message || error);
    process.exit(1);
  } finally {
