...
async function teardown() {
  ...
  try {
    ...
    const workspaces = await prisma.workspace.findMany({
      where: {
        OR: [{ name: "SEC_TEST_VICTIM_WS" }, { name: "SEC_TEST_ATTACKER_WS" }],
      },
    });
    ...
    if (workspaceIds.length > 0) {
      ...
      console.log(
        "Workspaces and related records (Properties, Tenants, Payments, Members) deleted.",
      );
    }
    ...
    for (const u of usersToDelete) {
      ...
      try {
        ...
        /* Already deleted by cascade or didn't exist */
      }
    }
