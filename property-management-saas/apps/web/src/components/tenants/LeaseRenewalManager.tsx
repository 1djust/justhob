...
export function LeaseRenewalManager({ workspaceId }: { workspaceId: string }) {
  ...
  const { data: renewals = [], isLoading: loading } = useQuery<any[]>({
    ...
    queryFn: async () => {
      const data = await apiFetch(
        `${API_BASE_URL}/api/workspaces/${workspaceId}/leases/renewals`,
        {
          credentials: "include",
        },
      );
  ...
  const reviewMutation = useMutation({
    mutationFn: async ({
      ...
      await apiFetch(
        `${API_BASE_URL}/api/workspaces/${workspaceId}/leases/renewals/${renewalId}/review`,
        {
          method: "PATCH",
  ...
  if (loading)
    return (
      <div className="animate-pulse h-32 bg-zinc-100 dark:bg-zinc-900 rounded-3xl" />
    );
    ...
    <div className="mb-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {renewals.map((r) => (
          <div
            key={r.id}
            className="p-5 rounded-2xl bg-amber-50/50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 relative overflow-hidden group"
          >
