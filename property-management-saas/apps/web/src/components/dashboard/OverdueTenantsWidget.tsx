...
export function OverdueTenantsWidget({ workspaceId }: { workspaceId: string }) {
  ...
  return (
    <div className="rounded-[2.5rem] border border-white/20 dark:border-zinc-800 bg-white/50 dark:bg-zinc-900/50 backdrop-blur-md p-8 shadow-2xl" aria-label="Overdue tenants">
      <div className="flex items-center gap-3 mb-6">
        <div className="p-2 bg-red-500/10 text-red-500 rounded-xl">
          <AlertCircle className="w-6 h-6" />
        </div>
        <h3 className="text-xl font-bold text-zinc-900 dark:text-white">Attention Needed: Overdue Rent</h3>
      </div>

      <div className="space-y-4">
        {payments.map(payment => {
          const isGraceEnded = payment.gracePeriodEnd && new Date(payment.gracePeriodEnd) <= new Date();
          const balance = payment.amount - (payment.amountPaid || 0);

          return (
            <div key={payment.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-2xl bg-white dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700">
              <div>
                <p className="font-bold text-zinc-900 dark:text-white">{payment.lease.tenant.name}</p>
                <p className="text-sm text-zinc-500">{payment.lease.property.name}</p>
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  ...
                  <p className="text-xs font-medium text-zinc-500 flex items-center justify-end gap-1 mt-1">
