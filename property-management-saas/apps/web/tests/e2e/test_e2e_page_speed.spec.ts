...
test("measure page load speeds as manager_pro", async ({ page }) => {
  ...
  const dashboardHeading = page
    .locator('h2:has-text("Dashboard"), h3:has-text("Dashboard")')
    .first();
  await dashboardHeading.waitFor({ state: "attached" });
  const dashboardLoadTime = Date.now() - submitStart;
  console.log(
    `[E2E] Login submit -> Dashboard fully loaded in: ${dashboardLoadTime}ms`,
  );
  ...
  const propertiesContent = page
    .locator('button:has-text("Add Properties")')
    .first();
