...
test.describe('Real-time Manager Notifications', () => {
  ...
  test.beforeAll(async () => {
    ...
    try {
      ...
      console.error('Failed to run setup script. The database might be unreachable.');
      throw error;
    }
  });

  test('Manager receives lease expiry notification without page refresh', async ({ page, request }) => {
    ...
    const response = await request.post('http://localhost:3001/api/admin/trigger-crons', {
      data: {
    ...
    const notificationTitle = page.locator('text=Tenant Lease Expiring').first();
    await expect(notificationTitle).toBeVisible();
    
    console.log('Test Passed! The manager received the notification instantly without a refresh.');
  });
});
