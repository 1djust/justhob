...
async function main() {
  ...
  await (
    prisma as unknown as {
      errorLog: { create: (args: unknown) => Promise<void> };
    }
  ).errorLog.create({
    data: {
      level: "info",
  ...
  const logs = await (
    prisma as unknown as {
      errorLog: { findMany: (args: unknown) => Promise<unknown[]> };
    }
  ).errorLog.findMany({
    take: 5,
  ...
  logs.forEach((log: unknown) => {
    ...
    const timestamp =
      typedLog.createdAt instanceof Date
        ? typedLog.createdAt.toISOString()
        : typedLog.createdAt;
    console.log(
      `[${timestamp}] ${typedLog.level.toUpperCase()}: ${typedLog.message} (Source: ${typedLog.source})`,
    );
  });
