...
async function setupScenario() {
  ...
  console.log(`✅ Success! Lease for ${tenant.name} updated to expire on ${targetDate.toDateString()} (30 days from now).`);
  console.log('\n👉 Now, go to your Admin Dashboard and click "Run System Jobs Now"!');
}

setupScenario().catch(console.error).finally(() => process.exit(0));
