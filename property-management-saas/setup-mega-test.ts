...
async function setupMegaScenario() {
  ...
  if (!scenarioArg || !['90', '60', '30', '7', '1', '14', '21', '31'].includes(scenarioArg)) {
  ...
  if (supaAdmin) {
    await prisma.user.upsert({
      ...
      create: { id: supaAdmin.id, email: 'admin@justhob.com', name: 'Platform Admin', role: 'SUPER_ADMIN' }
    });
    console.log(`✅ admin@justhob.com synced to Prisma (UUID: ${supaAdmin.id})`);
  } else {
    console.warn('⚠️  Could not find admin@justhob.com in Supabase — skipping Prisma sync');
  }
  ...
  if (scenario === 90 || scenario === 60 || scenario === 30) {
    ...
    if (tenant && tenant.leases.length > 0) {
      ...
      if (scenario === 7) {
        ...
        targetDate.setDate(targetDate.getDate() - (scenario === 31 ? 30 : scenario));
      }
      targetDate.setHours(0, 0, 0, 0);
      ...
      await prisma.payment.create({
        data: {
          ...
          ...(scenario === 31 ? { 
            evictionDate: new Date(new Date().getTime() + 7 * 24 * 60 * 60 * 1000)
          } : {})
        }
      });
      console.log(`💰 ${tenantEmail}: Created payment ${scenario === 7 ? 'due in 7 days' : `overdue by ${scenario} days`}.`);
    }
  }
  ...
  const landlordUser = await prisma.user.findUnique({ where: { email: 'landlord@justhob.com' } });
  ...
  if (landlordUser && tenant) {
    ...
    console.log('✅ Landlord bank details updated (Emerald Gardens Property / Access Bank)');
  }
